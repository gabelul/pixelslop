...
3. **Extract the fields you need:**
   ...
   - `viewports.desktop.typographyMetrics` **and** `viewports.mobile.typographyMetrics` — derived readability metrics measured off real layout at both viewports: `bodyCharsPerLine`, `bodyFontSize`, `bodyLeadingRatio`, `bodyTrackingEm`, `justifiedBody`, `allCapsBody`, `typeScaleRatio`, `flatHierarchy`, `oversizedH1`/`h1FontSize`, `bodyAtViewportEdge`. Typography fails differently per viewport — a comfortable 70-char desktop line becomes a 95-char wall on mobile, and body text that clears the floor on desktop can drop under 14px on a phone. Grade the **worse** of the two, and say which viewport it fired on.
...
4. **Apply the rubric** from scoring.md (Pillar 2: Typography). Evaluate each criterion:
   ...
   - **Measured readability** — `typographyMetrics` gives you hard numbers **at both desktop and mobile**. Check each viewport against the scoring.md thresholds and emit a finding for the worse one, naming the viewport it fired on: `bodyCharsPerLine` outside 45–85 (measure), `bodyFontSize` under 14px (below floor), `bodyLeadingRatio` under 1.3 or over 1.9 (leading), `bodyTrackingEm` under -0.02 or over 0.1 (tracking), `flatHierarchy` true (type scale too flat), `justifiedBody` true, `allCapsBody` true, `oversizedH1` true (shouty hero), `bodyAtViewportEdge` true (text flush to the edge). A metric that passes on desktop but fails on mobile is still a fail — that's the whole reason we measure both. Two or more compounding on either viewport = Score 1 territory. If a viewport's `typographyMetrics` is null or its fields are null, grade off the one you have and lower confidence — don't invent the numbers.
...
- `criterion` — which typography aspect (font-choice, type-scale, weight-discipline, line-height, letter-spacing, font-count, font-loading, measure, body-size, leading, tracking, flat-hierarchy, justified-body, all-caps-body, oversized-h1, body-at-edge)
